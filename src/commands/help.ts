...
const command: CommandInterface = {
	...
	execute: async (args: ArgumentsInterface): Promise<void> => {
		...
		const Line = CLI.Line;
		new Line().column('Options', 60, [clc.cyan]).column('Description', 40, [clc.blackBright]).fill().output();
		...
		new Line().column('Commands', 60, [clc.green]).column('Description', 40, [clc.blackBright]).fill().output();
