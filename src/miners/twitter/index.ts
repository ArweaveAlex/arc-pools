...
export async function processIdsV2(
 ...
 **/
export async function processThreadV2(
	poolClient: IPoolClient,
	serviceClient: ServiceClient,
	args: {
		tweet: any;
	...
	if (thread && thread.length > 0) {
		...
		const threadTweets = await getTweetsfromIds(poolClient, serviceClient, {
			ids: thread.map((tweet: any) => tweet.id),
		});
...
async function getThread(
	...
	do {
		const response = await axios.get(conversationEndpoint(args.conversationId, paginationToken), {
			headers: {
	...
	if (args.tweet && args.tweet.referenced_tweets) {
		for (let i = 0; i < args.tweet.referenced_tweets.length; i++) {
			if (args.tweet.referenced_tweets[i].type && args.tweet.referenced_tweets[i].type === 'quoted') {
				...
				const fetchedTweets: any[] = await getTweetsfromIds(poolClient, serviceClient, {
					ids: [args.tweet.referenced_tweets[i].id],
				});
...
async function processProfileImage(args: { tweet: any; tmpdir: any }) {
		...
		try {
			...
			for (let i = 0; i < args.tweet.includes.media.length; i++) {
				...
				if (
					...
					(mediaObject.type === 'video' || mediaObject.type === 'animated_gif')
				) {
					const variants = mediaObject?.variants.sort((a: any, b: any) =>
						(a.bitrate ?? 1000) > (b.bitrate ?? 1000) ? -1 : 1
					);
