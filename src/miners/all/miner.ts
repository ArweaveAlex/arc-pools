...
export async function run(poolConfig: PoolConfigType) {
	...
	const runMiners = async (validatedMiners: ValidatedMinerType[]) => {
		await Promise.all(
			validatedMiners.map(async (miner: ValidatedMinerType) => {
				await miner.run();
			})
		);
	};
...
async function getValidatedMiners(poolClient: IPoolClient, poolConfig: PoolConfigType): Promise<ValidatedMinerType[]> {
